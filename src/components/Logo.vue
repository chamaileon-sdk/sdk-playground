...
export default {
  ...
  methods: {
    createChamaileonLogo(config) {
      config = config || {};
      var eyeR = config.eyeR || 5.9367;
      var withLogo =
        typeof config.withLogo === 'undefined' ? true : config.withLogo;
      ...
      var iDot = document.createElementNS(
        'http://www.w3.org/2000/svg',
        'circle'
      );
      ...
      var pupil = document.createElementNS(
        'http://www.w3.org/2000/svg',
        'circle'
      );
      ...
      function setEyeProps(x, y) {
        ...
        if (changeColors) {
          svg.style.fill =
            'rgb(200, 200, ' +
            Math.floor(dist > 255 ? 255 : Math.floor(dist)) +
            ')';
        }
